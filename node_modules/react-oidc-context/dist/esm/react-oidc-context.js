...
var AuthProvider = (props) => {
    ...
    const removeUser = useCallback(
        userManager ? () => userManager.removeUser().then(onRemoveUser) : unsupportedEnvironment("removeUser"), [userManager, onRemoveUser]
    );
    const signoutRedirect = useCallback(
        (args) => userManagerContext.signoutRedirect(args).then(onSignoutRedirect), [userManagerContext.signoutRedirect, onSignoutRedirect]
    );
    const signoutPopup = useCallback(
        (args) => userManagerContext.signoutPopup(args).then(onSignoutPopup), [userManagerContext.signoutPopup, onSignoutPopup]
    );
    const signoutSilent = useCallback(
        (args) => userManagerContext.signoutSilent(args), [userManagerContext.signoutSilent]
    );
